...
compila o IG na imagem oficial **`hl7fhir/ig-publisher-base`** e publica `output/` no
**GitHub Pages**:

- **Gatilho:** push na `main` ou execução manual (`workflow_dispatch`).
- **Job `build`:** baixa `publisher.jar` → instala SUSHI (`npm install -g fsh-sushi`, ver HEL-007
  herdada) → `java -jar publisher.jar publisher -ig .` → falha se o QA tiver erros →
...
Site final: `https://<owner>.github.io/familiarativa-ig/`.
